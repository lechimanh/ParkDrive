...
export default {
    ...
    methods: {
        ...
        async search() {
            ...
            let params = {
                name : this.dataSearch.name ? this.dataSearch.name: null,
                organization : this.dataSearch.organization ? this.dataSearch.organization : null,
            }
            const res = await UserApi.getListUser(JSON.stringify(params));
            this.result = res.data
        ...
        redirectPageCreate() {
            this.$router.push(`/user/create-user`);
        },
        resetDataSearch(){
            this.dataSearch.name = null;
            this.dataSearch.organization = 0;
        }
    },
}
</script>
