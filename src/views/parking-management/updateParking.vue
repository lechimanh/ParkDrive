<template>
    <div ref="outSideClickNews">
        <h2 class="intro-y text-lg font-medium mt-4 mb-5">Cập nhật bãi đỗ xe</h2>
        <div>
            <ModalBody>
                <Form class="formNews" @submit="updateParking" :validation-schema="schema" v-slot="{ errors }" enctype="multipart/form-data">
                    <div class="intro-y mx-auto w-11/12 p-3 box">
                        ...
                        <div class="flex flex-row justify-center gap-5 pl-5 pr-5 form-create">
                            ...
                            <div class="w-1/2 create-layout">
                                <label for="regular-form-1" class="form-label text-base">Địa chỉ:</label>
                                <Field
                                    type="text"
                                    name="address"
                                    class="form-control pr-10 w-full"
                        ...
                        <div class="flex flex-row justify-center gap-5 pl-5 pr-5 form-create">
                            <div class="w-1/2 create-layout">
                                <label for="regular-form-1" class="form-label text-base">Giờ bất đầu:</label>
                                <Field
                                    type="text"
                                    name="openingHours"
                                    class="form-control pr-10 w-full"
                            ...
                            <div class="w-1/2 create-layout">
                                <label for="regular-form-1" class="form-label text-base">Giờ kết thúc:</label>
                                <Field
                                    type="text"
                                    name="closingHours"
                                    class="form-control pr-10 w-full"
                        ...
                        <div class="flex flex-row justify-center gap-5 pl-5 pr-5 form-create">
                            <div class="w-full create-layout">
                                <label for="regular-form-1" class="form-label text-base">Chi tiết:</label>
                                <Field
                                    as="textarea"
                                    type="text"
                                    rows="6"
                        ...
                        <div class="intro-x mt-4 mx-auto text-center">
                            <button class="btn btn-secondary w-24 mr-4 mb-2" @click="goToListParking()">Hủy</button>
                            <button class="btn btn-primary w-24 mb-2" type="submit">Cập nhật</button>
                        </div>
                    </div>
                </Form>
            </ModalBody>
        ...
        <div id="success-notification-content" class="toastify-content toastify-content-update hidden flex">
            ...
            <div class="ml-1 mr-2 message-toast">
                <div class="text-slate-500">Cập nhật thành công</div>
            </div>
        </div>
    </div>
</template>
...
import { updateParking, getParking } from '@/firebase'
...
export default {
    ...
    methods: {
        async updateParking() {
            const idParking = new URLSearchParams(window.location.search).get('id')
            await updateParking(idParking, { ...this.parkingLots })
            Toastify({
            ...
            this.$router.push('/parking-management/list-parking')
        },

        async getParking() {
            const idParking = new URLSearchParams(window.location.search).get('id')
            const park = await getParking(idParking)
            ;(this.parkingLots.name = park.name),
                (this.parkingLots.address = park.address),
                (this.parkingLots.email = park.email),
                (this.parkingLots.phone = park.phone),
                (this.parkingLots.openingHours = park.openingHours),
                (this.parkingLots.closingHours = park.closingHours),
                (this.parkingLots.desc = park.desc)
            console.log(park)
        },
        goToListParking() {
            this.$router.push('/list-parking')
        }
    }
}
</script>
